...
class DualOctree:
    ...
    def add_self_loops(self):
        for d in range(self.full_depth, self.depth + 1):
            ...
            self.graph[d] = {'edge_idx': torch.stack([torch.cat([row, unique_idx]), 
                torch.cat([col, unique_idx])]), 'edge_dir': torch.cat([edge_dir, dir_idx])}
    ...
    def remap_node_idx(self):
        ...
        for d in range(self.full_depth, self.depth + 1):
            ...
            # print(f"remap shape: {remap.shape}")
            # print(f"max edge_idx: {self.graph[d]['edge_idx'].max()}, min edge_idx: {self.graph[d]['edge_idx'].min()}")
            # print(f"remap indexing range: {remap.shape[0]}")
    ...
    def get_input_semantic_feature(self):
        ...
        offset = 0
        # for d in range(self.full_depth, self.depth):
        for d in range(self.full_depth+1, self.depth):
            full_features = self.octree.semantic_flatten[d]
        ...
        # === 4. Concatenate with finest level ===
        finest_feature = self.octree.semantic_flatten[self.depth]
        return torch.cat([coarse_feature, finest_feature], dim=0).unsqueeze(-1)
