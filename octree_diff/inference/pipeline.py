...
This script is the indoor path.
    ...
    # unconditional
    python -m octree_diff.inference.pipeline --mode generate \
        --stages both --num_samples 8 --out out/gen

    # completion from a partial scene
    python -m octree_diff.inference.pipeline --mode inpaint \
        --input scene.pt --known-mask known.pt --out out/complete

    # extend along +x
    python -m octree_diff.inference.pipeline --mode outpaint \
...
from octree_diff.octree.util_dualoctree import octree2split_small, split2octree_small
