...
from octree_diff.models.factory import build_structure_vae, build_structure_unet
...
def train(config_dir=None, device=None):
    # Load configurations
    full_d_cfg, full_vae_cfg = load_stage("str_diff", config_dir)
    ...
    vae = build_structure_vae(vae_cfg, device)
    ...
    model = build_structure_unet(d_cfg, device)
    ...
    latent_scale = resolve_latent_scale(
        ...
        os.path.splitext(t_cfg["checkpoint_path"])[0] + "_meta.yaml", "structure")

    ema = EMA(model, decay=t_cfg["ema_decay"])
    total_steps = t_cfg["epochs"] * len(loader)
    warmup = t_cfg["warmup_steps"]
    step = 0

    print(f"Starting Structure Diffusion training on {device}...")
    print(f"{len(loader)} steps/epoch x {t_cfg['epochs']} epochs = {total_steps} steps")
    ...
    for epoch in range(t_cfg["epochs"]):
        ...
        torch.save(model.state_dict(), t_cfg["checkpoint_path"].replace(".pt", "_raw.pt"))
