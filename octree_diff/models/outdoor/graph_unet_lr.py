...
from octree_diff.models.common.ldm_diffusion_util import conv_nd, default
from octree_diff.models.common.modules import (
    AttentionBlock,
    ConvDownsample,
    ConvUpsample,
...
class UNet3DModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.input_emb = conv_nd(
            dims, 2 * self.in_channels, self.model_channels, 3, padding=1
        )
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        ...
                        (
                            ConvDownsample(dim_out, dims=dims)
                            if not is_last
                            else our_Identity()
                        ),
                    ]
        ...
        self.end = nn.Sequential(
            convnormalization(self.model_channels), activation_function()
        )
    ...
    def forward_as_middle(self, h, doctree, timesteps, label, context):
        ...
        h_lr = self.forward(
            x=h_lr, timesteps=timesteps, label=label, context=context, as_middle=True
        )
...
if __name__ == "__main__":
    ...
    # Forward pass
    with torch.no_grad():
        out = model(x=x, timesteps=timesteps)
