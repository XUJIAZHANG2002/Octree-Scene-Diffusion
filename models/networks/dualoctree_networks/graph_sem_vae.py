...
class GraphVAE(torch.nn.Module):
    ...
    def octree_decoder(self, code, doctree_out,  update_octree=False):
        ...
        for i, d in enumerate(
            ...
            if d == self.depth_out:
                ...
                sem_vox = self.sem_predict([deconvs[d], doctree_out, d])
                nnum = doctree_out.nnum[d]
    ...
    #         nnum = doctree_out.nnum[d]
    #         logits[d] = logit[-nnum:]
    #         # print("logit shape", logits[d].shape)
            
    #         if d > self.depth_stop:
    #             nnum = doctree_pred.nnum[d - 1]
    #             leaf_mask = doctree_pred.node_child(d - 1) < 0
    #             deconvs_pred = (deconvs)
    #             deconvs_pred[d] = self.upsample[i - 1](
    #                 deconvs_pred[d - 1], doctree_pred, d, leaf_mask, nnum
    #             )           
    ...
    #             sem_vox = self.sem_predict([deconvs_pred[d], doctree_pred, d])
    #             nnum = doctree_pred.nnum[d]
    #             sem_voxs[d] = sem_vox[-nnum:]
    ...
    #             doctree_out = dual_octree.DualOctree(octree_out)
    #             doctree_out.post_processing_for_docnn()
    #             doctree_pred = doctree_out
    #         else:
    #             #train
    #             # print("train, split trees")
   
    #             label = logits[d].argmax(1).to(torch.int32)
    #             octree_pred.octree_split(label, d)
    #             if d < self.depth_out:
    #                 octree_pred.octree_grow(
    #                     d + 1
    #                 )  # 对初始化的满八叉树，根据预测的标签向上增长至depth_out
    #                 octree_pred.depth += 1
    #             doctree_pred = dual_octree.DualOctree(octree_pred)
       
    #                 #doctree_pred.post_processing_for_docnn()               
    ...
    #     else:
    #         #train
    #         return logits, sem_voxs, octree_pred
