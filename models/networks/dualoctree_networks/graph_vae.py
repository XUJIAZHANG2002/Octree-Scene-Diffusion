...
class GraphVAE(torch.nn.Module):
    ...
    def __init__(
        ...
        NUM_CLASSES = 30
        ## added 6.23 2025
        self.embedding = torch.nn.Embedding(num_embeddings=NUM_CLASSES, embedding_dim=channel_in)
    ...
    # def _get_input_feature(self, doctree):
    #     # Get original features (only for depth=self.depth)
    #     features = doctree.octree.features[self.depth].squeeze(-1).long()
        
    #     # Clamp feature indices to valid embedding range
    #     max_id = self.embedding.num_embeddings - 1
    #     features = torch.clamp(features, min=0, max=max_id)

    #     embedded = self.embedding(features)

    #     # Match the expected full number of leaf nodes (all depths up to self.depth)
    #     if True:  # all_leaf_nodes=True logic
    #         channel = embedded.shape[1]
    #         leaf_num = doctree.lnum[doctree.full_depth:self.depth].sum()
    #         zeros = torch.zeros(leaf_num, channel, device=embedded.device)
    #         embedded = torch.cat([zeros, embedded], dim=0)
    ...
    def octree_encoder(
        ...
        # print("postierior shape", code.shape)
        return posterior
    ...
    def octree_decoder(self, code, doctree_out, update_octree=False):
        ...
        for i, d in enumerate(
            ...
            if (
                update_octree
            ):  # 测试阶段：如果update_octree为true，则从full_depth开始逐渐增长八叉树，直至depth_out
    ...
    def forward(
        self, octree_in, octree_out=None, pos=None, evaluate=False
        ...
        print("octree.full_depth =", octree_in.full_depth)
        for d in range(octree_in.depth + 1):
            print(f"children[{d}] is None? ->", octree_in.children[d] is None)
        ...
        output["neural_mpu"] = (
            _neural_mpu  # 这个output['neural_mpu']主要用于测试阶段，相当于对于任意输入的pos，根据最后一层的reg_voxs返回pos对应的sdf值。
        )
