...
def gaussian_kernel(size, sigma):
    ...
    Args:
        size (int): The side length of the square kernel (e.g., 128).
        sigma (float): The standard deviation of the Gaussian distribution.

    Returns:
        np.ndarray: A 2D Gaussian kernel of shape (size, size).
    """
...
class SemKITTI(data.Dataset):
    ...
    def _get_road_mask(self, voxel_label):
        return (voxel_label == 9).max(axis=2)
    
    def _random_mask_index(self, mask, weight=None):
        if weight is not None:
            xx = np.linspace(-3, 3, mask.shape[0])
            yy = np.linspace(-3, 3, mask.shape[1])
            weight = np.outer(weight(xx), weight(yy))
            mask = mask * weight
        mask = mask / mask.sum()

        # 1. Flatten the probability distribution
        flat_mask = mask.flatten()

        # 2. Sample a single index from the flattened array
        # The 'a' argument here is np.arange(flat_mask.size) to represent the indices
        # of the flattened array.
        sampled_flat_index = np.random.choice(np.arange(flat_mask.size), size=1, p=flat_mask)[0]

        # 3. Unravel the index back to multi-dimensional coordinates
        sampled_coords = np.unravel_index(sampled_flat_index, mask.shape)

        return sampled_coords
    
    def _shift_center(self, mask, center, crop_size=(128, 128)):
        xmin, ymin = 0, 0
        xmax, ymax = mask.shape
        x, y = center
        dx, dy = crop_size
        x = max(min(x, xmax - dx), xmin + dx)
        y = max(min(y, ymax - dy), ymin + dy)
        return x, y
